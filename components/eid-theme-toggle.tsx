...
function IconMoon({ className }: { className?: string }) {
  return (
    <svg className={`h-4 w-4 shrink-0 ${className ?? ""}`} viewBox="0 0 24 24" width={18} height={18} aria-hidden>
      <path
        fill="currentColor"
...
export function EidThemeToggle({ className, variant = "default" }: Props) {
  ...
  const toolbarCls =
    variant === "toolbar"
      ? "inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-xl border border-[color:var(--eid-border-subtle)] bg-eid-card/85 p-0 text-eid-primary-300 shadow-sm transition hover:border-eid-primary-500/35 hover:bg-eid-surface hover:text-eid-fg focus:outline-none focus-visible:ring-2 focus-visible:ring-eid-primary-500/50 [touch-action:manipulation] sm:h-9 sm:w-9"
