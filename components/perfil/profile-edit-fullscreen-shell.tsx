...
export function ProfileEditFullscreenShell({ title, subtitle, backHref, children, topAction, showBack = true }: Props) {
  return (
    <main className="mx-auto w-full max-w-3xl px-3 pb-[max(1.5rem,env(safe-area-inset-bottom,0px))] pt-[calc(0.8rem+max(4.2rem,env(safe-area-inset-top,0px)))] sm:px-5 sm:pt-[calc(0.7rem+max(4rem,env(safe-area-inset-top,0px)))]">
