...
export function ProfileEidPerformanceSeal({ notaEid, className = "", title = "Nota EID no esporte", compact }: Props) {
  ...
  if (compact) {
    return (
      <div
        className={`inline-flex shrink-0 items-center rounded-full border border-eid-primary-500/45 text-[6px] font-black uppercase leading-none text-white shadow-[0_1px_2px_rgba(0,0,0,0.22)] sm:text-[6.5px] ${className}`}
        title={title}
      >
        <span className="rounded-l-full bg-black px-[4px] py-px sm:px-[4.5px]">EID</span>
        <span className="rounded-r-full bg-eid-primary-500 px-[4px] py-px tabular-nums sm:px-[4.5px]">{v.toFixed(1)}</span>
      </div>
    );
  }
  return (
