  ...
  return (
    <div className="mt-2 flex flex-wrap items-center gap-3">
      ...
      {mounted && editorPresent && previewUrl
        ? createPortal(
            <div
              className={`fixed inset-0 z-[96] flex items-center justify-center bg-black/60 px-4 transition-opacity duration-180 ${editorClosing ? "opacity-0" : "opacity-100"} motion-safe:animate-[fade-in_180ms_ease-out_both]`}
            >
              <button type="button" aria-label="Fechar editor" className="absolute inset-0" onClick={() => setEditorOpen(false)} />
              <div
                className={`relative z-[1] w-full max-w-xs rounded-2xl border border-[color:var(--eid-border-subtle)] bg-eid-card p-3 shadow-xl transition-all duration-180 motion-safe:animate-[eid-content-block-enter_240ms_cubic-bezier(0.22,1,0.36,1)_both] ${editorClosing ? "translate-y-2 scale-[0.985] opacity-0" : "translate-y-0 scale-100 opacity-100"}`}
              >
                ...
                <div className="mt-2 grid gap-2">
                  <label className="block text-[10px] text-eid-text-secondary">Zoom<input type="range" min={1} max={2.4} step={0.05} value={zoom} onChange={(ev) => setZoom(Number(ev.target.value))} className="mt-1 w-full" /></label>
                  <label className="block text-[10px] text-eid-text-secondary">Posição horizontal<input type="range" min={-100} max={100} step={1} value={posX} onChange={(ev) => setPosX(Number(ev.target.value))} className="mt-1 w-full" /></label>
                  <label className="block text-[10px] text-eid-text-secondary">Posição vertical<input type="range" min={-100} max={100} step={1} value={posY} onChange={(ev) => setPosY(Number(ev.target.value))} className="mt-1 w-full" /></label>
                </div>
                <div className="mt-2 flex justify-end gap-2">
                  <button type="button" onClick={() => setEditorOpen(false)} className="rounded-lg border border-[color:var(--eid-border-subtle)] px-2 py-1 text-[10px] font-semibold text-eid-text-secondary">
                    Cancelar
                  </button>
