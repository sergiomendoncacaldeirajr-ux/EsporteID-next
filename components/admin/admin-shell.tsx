...
export function AdminShell({
  ...
  return (
    <div className="min-h-screen bg-eid-bg text-eid-fg">
      <header className="sticky top-0 z-40 border-b border-[color:var(--eid-border-subtle)] bg-eid-bg/90 shadow-[0_8px_32px_rgba(0,0,0,0.35)] backdrop-blur-md">
        ...
        <div className="mx-auto flex max-w-6xl flex-col gap-3 px-3 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-eid-primary-500/30 bg-eid-primary-500/10 text-sm font-black text-eid-primary-300">
              A
            </div>
            <div>
        ...
        {!hasServiceRole ? (
          <div className="border-t border-amber-500/25 bg-gradient-to-r from-amber-500/10 via-amber-500/5 to-amber-500/10 px-3 py-2.5 text-center text-xs leading-snug text-amber-100 sm:px-6">
            Defina <code className="rounded-md bg-eid-bg/80 px-1.5 py-0.5 font-mono text-[11px]">SUPABASE_SERVICE_ROLE_KEY</code> no{" "}
            <code className="rounded-md bg-eid-bg/80 px-1.5 py-0.5 font-mono text-[11px]">.env.local</code> para carregar listagens e
            salvar alterações.
          </div>
        ) : null}
        <AdminNav />
