...
function SideFace({ side, variant, interactive, showEid }: { side: ConfrontoSide; variant: "individual" | "formacao"; interactive: boolean; showEid: boolean }) {
  ...
  const avatar = (
    <span
      ...
      } bg-eid-surface shadow-[0_8px_22px_-16px_rgba(15,23,42,0.5)] transition ${interactive ? "group-hover:border-eid-primary-500/50" : ""}`}
    >
      {side.avatarUrl ? (
  ...
  return (
    <div className="flex min-w-0 flex-1 flex-col items-center">
      {interactive ? (
        ...
        <span className="relative block h-14 w-14 shrink-0">{avatar}</span>
      )}
      {showEid ? <EidMini value={side.eid} /> : null}
...
export function ConfrontoCard({ item }: { item: PublicConfronto }) {
  ...
  return (
    <article className="relative overflow-hidden rounded-2xl border border-[color:var(--eid-border-subtle)] bg-[linear-gradient(160deg,color-mix(in_srgb,var(--eid-card)_94%,var(--eid-primary-500)_6%),color-mix(in_srgb,var(--eid-surface)_92%,transparent))] shadow-[0_14px_36px_-30px_rgba(15,23,42,0.75)]">
      ...
      <div className={`${isEncerrado ? "pointer-events-none" : ""} relative z-[1] p-3`}>
        ...
        <div className="mt-3 grid grid-cols-[minmax(0,1fr)_4.25rem_minmax(0,1fr)] items-start gap-2">
          ...
          <SideFace side={item.ladoB} variant={formacao ? "formacao" : "individual"} interactive={isEncerrado} showEid={isEncerrado} />
        </div>
