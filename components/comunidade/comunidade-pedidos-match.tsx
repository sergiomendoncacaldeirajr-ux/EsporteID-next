...
export function ComunidadePedidosMatch({ items }: { items: PedidoMatchItem[] }) {
  ...
  return (
    <div className="mt-3 space-y-3" onClick={() => setShowStatsHint(false)}>
      ...
      {err ? (
        <p className="rounded-lg border border-red-400/30 bg-red-500/10 px-3 py-2 text-xs font-semibold text-red-200 eid-light:border-red-300 eid-light:bg-red-50 eid-light:text-red-800">{err}</p>
      ) : null}
