...
function WhatsAppContatoButton({ href, nome }: { href: string; nome: string | null | undefined }) {
  ...
  return (
    <a
      ...
      className="flex min-h-[38px] w-full items-center justify-center gap-1.5 rounded-xl bg-[#25D366] px-3 text-[10px] font-black uppercase tracking-[0.05em] text-white shadow-[0_4px_14px_-8px_rgba(37,211,102,0.5)] transition hover:bg-[#1fbb5a] active:scale-[0.98] active:brightness-95"
    >
      {WA_ICON}
    ...
    <div className="overflow-hidden rounded-xl border border-[rgba(37,99,235,0.16)] bg-[linear-gradient(145deg,color-mix(in_srgb,var(--eid-card)_97%,var(--eid-primary-500)_3%),color-mix(in_srgb,var(--eid-surface)_95%,transparent))] shadow-[0_4px_16px_-8px_rgba(15,23,42,0.3),inset_0_1px_0_rgba(255,255,255,0.03)]">
      ...
      <div className="m-2.5 space-y-1.5 md:m-3 md:space-y-2">
        ...
        {items.map((m) => {
          ...
          const avatarFrameClass = `inline-flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden border border-[color:var(--eid-border-subtle)] bg-eid-surface text-sm font-black text-eid-primary-300 ${avatarShapeClass}`;
                ...
                <div className="grid gap-2" style={{ gridTemplateColumns: m.whatsappContato && podeReagendar ? "1fr 1fr" : "1fr" }}>
                  ...
                  {podeReagendar ? (
                    <button
                      ...
                      className="flex min-h-[38px] w-full items-center justify-center gap-1.5 rounded-xl border border-eid-action-500/45 bg-eid-action-500/10 px-3 text-[10px] font-black uppercase tracking-[0.05em] text-[color:color-mix(in_srgb,var(--eid-fg)_72%,var(--eid-action-500)_28%)] transition hover:border-eid-action-500/65 hover:bg-eid-action-500/18 active:scale-[0.98] disabled:opacity-50"
                    >
                      <svg viewBox="0 0 16 16" className="h-3.5 w-3.5 shrink-0 text-eid-action-400" fill="currentColor" aria-hidden>
