...
export function mapPaymentStatus(raw?: string | null) {
...
/**
 * Mesma regra aplicada ao webhook real do Asaas (atualiza cobranças de professor, espaço e assinatura PaaS).
 */
export async function processAsaasWebhookPayload(payload: AsaasWebhookPayload | null) {
  ...
  async function sendNotificationAndPush(payloadNotif: {
    ...
    if (!payloadNotif.usuario_id) return;
    const { data } = await admin
      .from("notificacoes")
      .insert({
        usuario_id: payloadNotif.usuario_id,
        mensagem: payloadNotif.mensagem,
        tipo: payloadNotif.tipo,
        referencia_id: payloadNotif.referencia_id ?? null,
        lida: false,
        remetente_id: payloadNotif.remetente_id ?? null,
        data_criacao: new Date().toISOString(),
      })
      .select("id")
      .limit(1);
    await triggerPushForNotificationIdsBestEffort([Number((data?.[0] as { id?: number } | undefined)?.id ?? 0)], {
      source: "lib/asaas-webhook-handler",
    });
  }
    ...
    if (mapped.pagamento === "received" && parceiroUsuarioId) {
      ...
      if (transacao && transacao.tipo !== "mensalidade_plataforma_espaco") {
        await admin.from("extrato_lancamentos").insert({
          ...
          comissao_plataforma: Number(transacao.comissao_plataforma_centavos ?? 0) / 100,
          valor_liquido_parceiro: Number(transacao.valor_liquido_espaco_centavos ?? 0) / 100,
