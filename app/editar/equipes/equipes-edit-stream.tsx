...
export type EditarEquipesStreamProps = {
  viewerId: string;
  sp: { from?: string; embed?: string };
};

export async function EditarEquipesStream({ viewerId, sp }: EditarEquipesStreamProps) {
  ...
  for (const row of timesRows ?? []) {
    ...
    podeExcluirPorTime.set(tid, await podeExcluirFormacaoComoLider(supabase, tid, viewerId));
  }
