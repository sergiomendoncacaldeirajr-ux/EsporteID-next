...
export type EditarTimeStreamProps = {
  timeId: number;
  viewerId: string;
  sp: { from?: string; embed?: string; convidar?: string };
};

export async function EditarTimeStream({ timeId, viewerId, sp }: EditarTimeStreamProps) {
  const id = timeId;
  ...
  await requireContaPerfilPronto(supabase, viewerId, contaNextPath(`/editar/time/${id}`, sp));

  const { data: t } = await supabase
    .from("times")
    .select(
      "id, nome, username, bio, localizacao, escudo, criador_id, tipo, genero, disponivel_amistoso, vagas_abertas, aceita_pedidos, nivel_procurado"
    )
    .eq("id", id)
    .maybeSingle();
  if (!t) notFound();
  if (t.criador_id !== viewerId) {
  ...
  const podeExcluirNestaPagina = await podeExcluirFormacaoComoLider(supabase, id, viewerId);
