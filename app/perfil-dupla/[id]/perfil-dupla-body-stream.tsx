...
import {
  ...
  PerfilFormacaoBodyConquistasSkeleton,
...
import {
  getPerfilDuplaHistEidPlayers,
  getPerfilDuplaIdentity,
  getPerfilDuplaPartidasBundle,
  getPerfilDuplaVisitorMatchPack,
} from "./perfil-dupla-payload";

export type PerfilDuplaBodyStreamProps = {
  duplaId: number;
  viewerId: string;
};
    ...
    <section className="overflow-hidden rounded-xl border border-transparent bg-eid-card/55 p-3">
      ...
      {!isMembroDupla ? (
        <div className="grid gap-3">
          {v.mostrarAvisoSemEidNoEsporteDupla ? (
          ...
          v.rankingBlockedUntilDupla ? (
            <p className="rounded-xl border border-[color:var(--eid-border-subtle)] bg-eid-surface/40 px-3 py-2 text-[11px] text-eid-text-secondary">
              ...
              <span className="font-semibold text-eid-fg">
                {new Date(v.rankingBlockedUntilDupla).toLocaleDateString("pt-BR")}
              </span>
              . <span className="font-semibold text-eid-fg">{formatCooldownRemaining(v.rankingBlockedUntilDupla)}</span>
            </p>
          ) : null}
          {v.canSugerirMatchDupla && v.viewerPodeConfrontarNesteEsporteDupla && timeResolvidoId ? (
            <div className="mt-3">
              <SugerirMatchLiderForm
                alvoTimeId={timeResolvidoId}
                alvoNome={timeResolvido?.nome ?? "Dupla no radar"}
                modalidadeLabel="dupla"
                formacoesMinhas={v.formacoesMembroNaoLiderDupla}
              />
            </div>
          ) : null}
        </div>
          ...
          <div className={`${PROFILE_CARD_BASE} mt-2 overflow-hidden p-3 sm:rounded-2xl sm:p-4`}>
            ...
            <div className="mt-2">
              ...
              <div className="mt-3 grid grid-cols-3 gap-2 border-t border-[color:var(--eid-border-subtle)] pt-3">
                ...
                <div className="rounded-2xl border border-[color:var(--eid-border-subtle)] bg-eid-surface/35 px-2 py-2.5 text-center shadow-[0_10px_24px_-18px_rgba(15,23,42,0.18)]">
                  <p className="text-base font-bold tabular-nums text-eid-fg sm:text-lg">{timeResolvido.pontos_ranking ?? 0}</p>
                ...
                <div className="rounded-2xl border border-[color:var(--eid-border-subtle)] bg-eid-surface/35 px-2 py-2.5 text-center shadow-[0_10px_24px_-18px_rgba(15,23,42,0.18)]">
                  <p className="text-base font-bold tabular-nums text-eid-primary-300 sm:text-lg">
                    {idn.posicaoDupla != null ? `#${idn.posicaoDupla}` : "—"}
                  </p>
...
export async function PerfilDuplaBodyGestaoMembro({ duplaId, viewerId }: PerfilDuplaBodyStreamProps) {
  const idn = await getPerfilDuplaIdentity(duplaId, viewerId);
  if (!idn.isMembroDupla) return null;
  const { fromPublicDupla, editarDuplaHref, isDonoDupla } = idn;
...
export async function PerfilDuplaBodyConquistas({ duplaId, viewerId }: PerfilDuplaBodyStreamProps) {
  const { conquistas } = await getPerfilDuplaIdentity(duplaId, viewerId);

  return (
    <ProfileSection title="Conquistas" info="Selos e marcos da dupla na plataforma (quando houver).">
      <ProfileAchievementsShelf achievements={conquistas} emptyText="Conquistas aparecerão conforme evolução da dupla." />
    </ProfileSection>
  );
}

export function PerfilDuplaBodyStream({ duplaId, viewerId }: PerfilDuplaBodyStreamProps) {
  return (
    <div className="mt-6 grid gap-6">
      <Suspense fallback={<PerfilFormacaoBodyVisitanteStackSkeleton />}>
        <PerfilDuplaBodyVisitanteStack duplaId={duplaId} viewerId={viewerId} />
      </Suspense>
      <Suspense fallback={<PerfilFormacaoBodyEidSkeleton />}>
        <PerfilDuplaBodyEid duplaId={duplaId} viewerId={viewerId} />
      </Suspense>
      <Suspense fallback={<PerfilFormacaoBodyConfrontosSkeleton />}>
        <PerfilDuplaBodyConfrontos duplaId={duplaId} viewerId={viewerId} />
      </Suspense>
      <Suspense fallback={<PerfilFormacaoBodyElencoSkeleton />}>
        <PerfilDuplaBodyParticipantes duplaId={duplaId} viewerId={viewerId} />
      </Suspense>
      <Suspense fallback={<PerfilFormacaoBodyGestaoSkeleton />}>
        <PerfilDuplaBodyGestaoMembro duplaId={duplaId} viewerId={viewerId} />
      </Suspense>
      <Suspense fallback={<PerfilFormacaoBodyConquistasSkeleton />}>
        <PerfilDuplaBodyConquistas duplaId={duplaId} viewerId={viewerId} />
      </Suspense>
    </div>
  );
}
