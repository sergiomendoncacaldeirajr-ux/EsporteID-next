...
export default async function ContaEditarTorneioPage({ params, searchParams }: Props) {
  ...
  const { data: t } = await supabase
    .from("torneios")
    .select(
      "id, nome, status, data_inicio, data_fim, banner, categoria, descricao, regulamento, premios, valor_inscricao, formato_competicao, criterio_desempate, regras_placar_json, criador_id, espaco_generico_id, esporte_id"
    )
    .eq("id", id)
    .maybeSingle();
  if (!t) notFound();
