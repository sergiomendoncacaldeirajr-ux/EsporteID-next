...
import {
  ...
  getPerfilTimeVisitorMatchPack,
} from "./perfil-time-payload";

export type PerfilTimeBodyStreamProps = {
  timeId: number;
  viewerId: string;
  removerMembroAction: (formData: FormData) => Promise<void>;
};

/** Convites de elenco + cartão “Ação principal” (só quem não é líder). */
export async function PerfilTimeBodyVisitanteStack({ timeId, viewerId }: { timeId: number; viewerId: string }) {
  const idn = await getPerfilTimeIdentity(timeId, viewerId);
  if (idn.isLeader) return null;

  const [pack, v] = await Promise.all([
    getPerfilTimeMembrosHistPack(timeId, viewerId),
    getPerfilTimeVisitorMatchPack(timeId, viewerId),
  ]);
  const { id, t, modalidade } = idn;
    ...
    <div className="space-y-4">
      <FormacaoElencoCallout>
        <FormacaoCandidaturaCta
          ...
          vagasDisponiveis={pack.vagasDisponiveis}
          minhaCandidaturaPendenteId={pack.minhaCandidaturaPendente?.id ?? null}
          jaSouMembro={idn.isMember}
          textAlign="start"
        />
      </FormacaoElencoCallout>
      ...
      {v.temBlocoAcaoVisitante ? (
        <div className="grid gap-3">
          ...
          {v.canChallenge &&
          v.viewerPodeConfrontarNesteEsporte &&
          !v.hasAceitoRank &&
          t.esporte_id &&
          !v.rankingBlockedUntilTime ? (
            <ProfilePrimaryCta
              href={`/desafio?id=${id}&tipo=${encodeURIComponent(modalidade)}&esporte=${t.esporte_id}`}
              label={v.linkWpp ? "⚡ Desafio no ranking" : undefined}
            />
          ) : null}
          {v.canChallenge && v.viewerPodeConfrontarNesteEsporte && !v.hasAceitoRank && v.rankingBlockedUntilTime ? (
            <p className="rounded-xl border border-[color:var(--eid-border-subtle)] bg-eid-surface/40 px-3 py-2 text-[11px] text-eid-text-secondary">
              ...
              <span className="font-semibold text-eid-fg">
                {new Date(v.rankingBlockedUntilTime).toLocaleDateString("pt-BR")}
              </span>
              . <span className="font-semibold text-eid-fg">{formatCooldownRemaining(v.rankingBlockedUntilTime)}</span>
            </p>
          ) : null}
        </div>
      ) : v.canChallenge && v.hasAceitoRank ? (
        <p className="text-xs text-eid-text-secondary">
          Desafio aceito nesta modalidade. Registre o resultado na agenda quando jogarem.
        </p>
      ) : (
        <p className="text-xs text-eid-text-secondary">
          Para desafiar direto, você precisa ser líder de uma {modalidade} neste esporte — ou use a sugestão abaixo se você
          já faz parte de uma formação.
        </p>
      )}
      {v.canSugerirMatch && v.viewerPodeConfrontarNesteEsporte ? (
        <div className="mt-3">
          <SugerirMatchLiderForm
            alvoTimeId={id}
            alvoNome={t.nome ?? "Formação"}
            modalidadeLabel={modalidade === "dupla" ? "dupla" : "equipe"}
            formacoesMinhas={v.formacoesMembroNaoLider}
          />
        </div>
      ) : null}
    </section>
...
export async function PerfilTimeBodyElenco({
  timeId,
  viewerId,
  removerMembroAction,
}: {
      ...
      {isLeader ? (
        <div className="mt-2 space-y-3">
          <p className="text-[11px] leading-relaxed text-eid-text-secondary">
            Convide por nome ou <span className="font-semibold text-eid-fg">@usuário</span>. Com três letras aparecem sugestões
            para escolher o atleta.
          </p>
...
export async function PerfilTimeBodyGestao({ timeId, viewerId }: { timeId: number; viewerId: string }) {
  const idn = await getPerfilTimeIdentity(timeId, viewerId);
  if (!idn.isLeader) return null;
  const { id, editarTimeHref } = idn;
...
export function PerfilTimeBodyStream({ timeId, viewerId, removerMembroAction }: PerfilTimeBodyStreamProps) {
  return (
    <>
      <Suspense fallback={<PerfilFormacaoBodyVisitanteStackSkeleton />}>
        <PerfilTimeBodyVisitanteStack timeId={timeId} viewerId={viewerId} />
      </Suspense>
      <div className="mt-6 grid gap-6">
        <Suspense fallback={<PerfilFormacaoBodyEidSkeleton />}>
          <PerfilTimeBodyEid timeId={timeId} viewerId={viewerId} />
        </Suspense>
        <Suspense fallback={<PerfilFormacaoBodyConfrontosSkeleton />}>
          <PerfilTimeBodyConfrontos timeId={timeId} viewerId={viewerId} />
        </Suspense>
        <Suspense fallback={<PerfilFormacaoBodyElencoSkeleton />}>
          <PerfilTimeBodyElenco timeId={timeId} viewerId={viewerId} removerMembroAction={removerMembroAction} />
        </Suspense>
        <Suspense fallback={<PerfilFormacaoBodyGestaoSkeleton />}>
          <PerfilTimeBodyGestao timeId={timeId} viewerId={viewerId} />
        </Suspense>
      </div>
    </>
  );
