...
export type PerfilTimeStreamProps = {
  timeId: number;
  viewerId: string;
...
export async function PerfilTimeStream({
  timeId,
  viewerId,
  sairEquipeAction,
  removerMembroAction,
}: PerfilTimeStreamProps) {
  const id = timeId;
  ...
  const podeExcluirPerfilFormacao =
    isLeader && (await podeExcluirFormacaoComoLider(supabase, id, viewerId));
