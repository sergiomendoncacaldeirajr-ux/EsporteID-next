    ...
    <div className="mx-auto max-w-2xl space-y-4">
      ...
      <section className="eid-mobile-section rounded-2xl border border-[color:var(--eid-border-subtle)] bg-eid-card/90 p-5">
        ...
        <p className="mt-2 text-sm text-eid-text-secondary">
          Para receber pagamentos no local, informe os dados da conta de recebimentos. O cadastro e a validação
          acontecem dentro do EsporteID, sem tirar o dono do painel.
        </p>
        <div className="mt-3 space-y-2 rounded-xl border border-eid-primary-500/25 bg-eid-primary-500/5 p-3">
          <p className="text-xs font-semibold text-eid-fg">Fluxo transparente</p>
          <p className="text-xs text-eid-text-secondary">1. O dono informa os dados abaixo.</p>
          <p className="text-xs text-eid-text-secondary">2. O EsporteID prepara a integração Asaas internamente.</p>
          <p className="text-xs text-eid-text-secondary">3. O admin acompanha as pendências no painel interno.</p>
        </div>
      </section>
      <section className="eid-mobile-section rounded-2xl border border-[color:var(--eid-border-subtle)] bg-eid-card/90 p-5">
