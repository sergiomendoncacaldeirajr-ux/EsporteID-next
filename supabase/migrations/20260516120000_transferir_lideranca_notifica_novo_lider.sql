-- Ao transferir liderança, notifica o novo líder no Social (histórico + push via app).
...
begin
  ...
  select t.criador_id,
         ...
         t.esporte_id,
         t.nome
    into v_old_leader, v_tipo, v_esp, v_nome
  from public.times t
  ...
  v_msg :=
    case
      when v_tipo = 'dupla' then
        'Você passou a ser líder da dupla "' || coalesce(v_nome, 'Formação') || '". O antigo líder transferiu a liderança para você.'
      else
        'Você passou a ser líder do time "' || coalesce(v_nome, 'Formação') || '". O antigo líder transferiu a liderança para você.'
    end;
