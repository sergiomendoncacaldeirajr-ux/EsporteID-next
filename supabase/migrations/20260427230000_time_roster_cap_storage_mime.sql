-- Limite de elenco: dupla máx. 2 integrantes (líder + elenco contado de forma distinta); time máx. 18.
-- Storage: tipos comuns que geravam "Bad Request" no bucket avatars (HEIC, pjpeg, octet-stream coberto no app).
...
revoke all on function public.time_roster_headcount(bigint) from public;
grant execute on function public.time_roster_headcount(bigint) to authenticated;
