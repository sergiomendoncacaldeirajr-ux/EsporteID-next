-- `can_challenge` em formações: mesmo critério do app em `radar-snapshot.ts` (líder OU membro ativo
-- com formação da modalidade no esporte). Antes só `criador_id`, o que quebrava casos em que o vínculo
-- via `membros_time` refletia `tipo = dupla` e o merge da página podia priorizar linha incompleta.
