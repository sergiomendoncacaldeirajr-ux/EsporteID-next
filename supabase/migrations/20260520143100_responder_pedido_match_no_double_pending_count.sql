-- Recusa de desafio (dupla/time): notificar elenco das duas formações além do capitão desafiante (já existia).
...
begin
      ...
      select
        (
          ...
          where m.finalidade = 'ranking'
            ...
            and not exists (
              select 1
              from public.partidas px
              where px.match_id = m.id
                and px.torneio_id is null
                and lower(trim(coalesce(px.status, ''))) in ('agendada', 'aguardando_confirmacao')
            )
        )
        +
        (
  ...
  if p_aceitar then
      ...
      if v_fin = 'ranking' then
        ...
        select
          ...
          case
            when v_acceptor_nome is not null and length(trim(v_acceptor_nome)) > 0
            then
              'O desafio de ranking foi recusado por '
              || trim(v_acceptor_nome)
              || ' (outra formação). Nenhuma partida foi agendada.'
            else
              'O desafio de ranking foi recusado pela outra formação. Nenhuma partida foi agendada.'
          end,
          'match',
          p_match_id,
          false,
        ...
        select
          ...
          case
            when v_acceptor_nome is not null and length(trim(v_acceptor_nome)) > 0
            then
              'O desafio amistoso foi recusado por '
              || trim(v_acceptor_nome)
              || ' (outra formação).'
            else
              'O desafio amistoso foi recusado pela outra formação.'
          end,
          'match',
          p_match_id,
          false,
    ...
    -- Membros do time desafiado (exceto o capitão que recusou)
    if v_adv_time_id is not null then
      if v_fin = 'ranking' then
        insert into public.notificacoes (
          usuario_id,
        ...
        select
          ...
          case
            when v_challenger_nome is not null and length(trim(v_challenger_nome)) > 0
            then
              'Seu capitão recusou o desafio de ranking enviado por '
              || trim(v_challenger_nome)
              || '. Nenhuma partida foi agendada.'
            else
              'Seu capitão recusou o desafio de ranking. Nenhuma partida foi agendada.'
          end,
          'match',
          p_match_id,
          false,
        ...
        select
          ...
          case
            when v_challenger_nome is not null and length(trim(v_challenger_nome)) > 0
            then
              'Seu capitão recusou o desafio amistoso enviado por '
              || trim(v_challenger_nome)
              || '.'
            else
              'Seu capitão recusou o desafio amistoso.'
          end,
          'match',
          p_match_id,
          false,
