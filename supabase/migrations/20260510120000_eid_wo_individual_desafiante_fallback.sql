-- Bug fix: partidas individuais W.O. sem placar numérico ficavam sem processamento.
-- Quando placar_1/placar_2/placar_desafiante/placar_desafiado são todos NULL (ou iguais),
-- a função retornava false antes de aplicar EID e pontos de ranking ao perdedor.
-- Correção: se v_is_wo = true e desafiante_id está preenchido e pertence à partida,
-- trata o desafiante como vencedor (desafiado forfeitou).
...
begin
  ...
  if v_side_1_score is not null and v_side_2_score is not null and v_side_1_score <> v_side_2_score then
    ...
    and v_partida.desafiante_id is not null
    and v_partida.desafiante_id in (v_partida.jogador1_id, v_partida.jogador2_id)
  then
    -- W.O. sem placar numérico: desafiante vence por ausência do desafiado
    v_winner_user := v_partida.desafiante_id;
    v_loser_user := case
      when v_partida.desafiante_id = v_partida.jogador1_id then v_partida.jogador2_id
      else v_partida.jogador1_id
    end;
  else
    return false;
  end if;
...
-- Mesma correção em aplicar_pontos_ranking_match_desafio: fallback W.O. desafiante para individual.
...
begin
  ...
  v_is_wo := public.eid_is_wo(
    v_row.tipo_partida,
    v_row.status_ranking,
    v_row.mensagem,
    v_row.resultado_json,
    v_row.placar
  );
  ...
  if v_collective then
    ...
    if v_s1 is not null and v_s2 is not null and v_s1 <> v_s2 then
      ...
      and v_row.desafiante_id is not null
      and v_row.desafiante_id in (v_row.jogador1_id, v_row.jogador2_id)
    then
      -- W.O. sem placar numérico: desafiante vence por ausência do desafiado
      v_winner_user := v_row.desafiante_id;
      v_loser_user := case
        when v_row.desafiante_id = v_row.jogador1_id then v_row.jogador2_id
        else v_row.jogador1_id
      end;
    else
      return;
    end if;
