-- Carência de ranking: partidas anuladas pelo admin não entram no cálculo da carência (RPC).
  ...
  if v_fin = 'ranking' and v_mod = 'individual' then
    ...
    if exists (
      ...
      where p.esporte_id = p_esporte_id
        ...
        and not (
          lower(trim(coalesce(p.status, ''))) in ('cancelada', 'cancelado')
          or lower(trim(coalesce(p.status_ranking, ''))) in ('cancelado_admin', 'anulado_admin')
        )
